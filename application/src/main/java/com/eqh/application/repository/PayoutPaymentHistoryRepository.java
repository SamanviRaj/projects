...
public interface PayoutPaymentHistoryRepository extends JpaRepository<PayoutPaymentHistory, Long> {
    ...
    @Query(value = "SELECT * FROM public.\"PAYOUT_PAYMENT_HISTORY\" pph WHERE pph.payee_party_number = :payeePartyNumber AND pph.payout_payee_id = :payoutPayeeId AND pph.reversed = false ORDER BY pph.id", nativeQuery = true)
